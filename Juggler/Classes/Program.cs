    ...
    static class Program
    {
        ...
        private static void InitializeApp()
        {
            ...
            Ticker.Tick +=
                delegate
                {
                    if (Settings.Default.ConfirmJuggle)
                    {
                        ShowConfirmJuggle();
                    }
                    else
            ...
            menuItems = new ToolStripItem[] 
                {
                    ...
                    new ToolStripMenuItem("Download Wallpapers", 
                        null, 
                        delegate 
                        { 
                            ShowWindow(DownloadForm.Instance);
                        }),
                    ...
                    new ToolStripMenuItem("About", 
                        ...
                        delegate 
                        { 
                            Process.Start("http://anand-prakash.net/juggler/default.aspx");
                        }),
                    new ToolStripMenuItem("Exit", 
                        null, 
                        delegate 
                        { 
                            if (DownloadForm.Instance.Crawler.CrawlStatus != CrawlStatus.Stopped)
                            {
                                DownloadForm.Instance.Crawler.Stop(StopReason.Exit, AppExit);
                            }
                            else 
                            {
                                AppExit();
        ...
        internal static void ShowConfirmJuggle()
        {
            Ticker.Stop();
            ConfirmForm win = new ConfirmForm();
            SetWindowPosition(win);
            win.Opacity = 0;
            win.TopMost = true;
            win.Show();
